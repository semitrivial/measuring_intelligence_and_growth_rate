...
Unfortunately, the original Hibbard measure is not computable, as the following
...
\label{originalnoncomputableprop}
    The original Hibbard measure is not computable. To be precise, fix a G\"odel
    numbering of predictors (for convenience, we may assume every natural number
    is a G\"odel number of some predictor).
    Let $F:\mathbb N\to\mathbb N$ be the function which takes as input a natural
    number $n$, say, a G\"odel number of a predictor with original Hibbard measure
    $m$, and outputs $m+1$ if $m<\infty$, or $0$ if $m=\infty$. Then $F$ is not
    computable.
\end{proposition}

\begin{proof}
    If $F$ were computable, then we could use $F$ to solve the halting problem,
    as follows.
    Let $p_0$ be the predictor which always outputs $0$, and let $m$ be its
    original Hibbard measure, clearly $m<\infty$. Let $p$ be a predictor with
    ...
    on the empty input, proceed as follows. Let $p_M$ be the predictor which, on
    input $(x_1,\ldots,x_n)$, outputs either the output of $p$ on said input,
    or else $0$, depending whether or not $M$ halts on empty input within
    $\leq n$ steps. Clearly, $M$ halts on empty input if and only if $p_M$ has
    original Hibbard measure $>m$.
\end{proof}
...
to be much good, it cannot possibly use real numbers to quantify growth rates (in a sense
we will make more precise). Essentially,
...
solutions to Problem \ref{bigoproblem} using alternative number systems to the reals,
which will yield ASPI measures (or taxonomies) in terms of those alternative number
systems.
...
intelligence levels of $Y$ and of $X_1,X_2,\ldots$ could not possibly be real numbers,
or else one of the $X_i$ would necessarily have an intelligence level higher than
$Y$. This was pointed out previously by \citet{ioi1},
and by \citet{alexander2020arch} again, the latter admist a wider discussion of
...
is a total order (where $x\geq_\mathcal U y$ if either $x>_\mathcal U y$ or $x=y$).
...
\begin{proof}
    For every evader $e$, let $N_e=\{n\in\mathbb N\,:\,t_e(n)<f(n)\}$.
    By Definition \ref{outgrowsdefn}, for every $e$, if the hyperreal growth rate of
    $t_e$ is $<\hat f$, then $N_e\in\mathcal U$.
...
We mention in passing that there are other ways to approach hyperreals
\citep{katz},
where the free ultrafilter dependency is replaced by a dependency on a model of
a certain set of axioms. We conjecture that
the latter dependency would be no less abstract than the former, at least for
our purposes here.
