...
\begin{itemize}
    ...
    quickly-growing functions. This recipe is called the \emph{slow-growing hierarchy}.
    \item
    The recipe takes as input an ordinal number along with a so-called \emph{fundamental
    sequence}, and outputs a quickly-growing computable function.
    \item
    Thus, instead of
    commanding our AGI to directly produce a list of quickly-growing computable functions,
    we can instead command our AGI to produce a list of computable ordinal numbers (carefully
    notated so as to include fundamental sequences), and then obtain a list of quickly-growing
...
\begin{definition}
    ...
    Suppose $\alpha$ is an ordinal and suppose $\mathscr F$ is a function which
    takes as input a limit ordinal $\lambda<\alpha$ and outputs a fundamental
    sequence $\mathscr F(\lambda)=(\lambda[0],\lambda[1],\lambda[2],\ldots)$
    for $\lambda$. The \emph{slow-growing hierarchy given by $\alpha$ and $\mathscr F$}
...
\begin{example}
    ...
    \[
        \mathscr F(\omega\cdot(n+1))(m) = (\omega\cdot(n+1))[m] = (\omega\cdot n)+m.
    \]
    Let us compute some of the functions $\{G_{\beta}\,:\,\beta<\alpha\}$
    given by $\alpha$ and $\mathscr F$. Below, $x$ and $n$
    denote arbitrary natural numbers.
    \begin{enumerate}
        \item
        By the base case, $F_0(x)=0$ for all $x\in\mathbb N$.
        \item
        By the successor case, $F_1(x)=F_0(x)+1=0+1=1$.
        \item
        By the successor case, $F_2(x)=F_1(x)+1=1+1=2$.
        \item
        Generalizing the above pattern, $F_n(x)=n$.
        \item
        By the limit case,
        $F_\omega(x)=F_{\omega\cdot(0+1)}(x)
        =F_{(\omega\cdot(0+1))[x]}(x)=F_{(\omega\cdot 0)+x}(x)
        =F_x(x)=x$ (by (4) above).
        \item
        By the successor case, $F_{\omega+1}(x)=F_{\omega}(x)+1=x+1$.
        \item
        By the successor case, $F_{\omega+2}(x)=F_{\omega+1}(x)+1=x+2$.
        \item
        Generalizing the above pattern, $F_{\omega+n}(x)=x+n$.
        \item
        By the limit case,
        $F_{\omega\cdot 2}(x)=F_{\omega\cdot(1+1)}(x)
        =F_{(\omega\cdot(1+1))[x]}(x)=F_{(\omega\cdot 1)+x}(x)
        =x+x$.
        \item
        Similarly, the reader can check that $F_{\omega\cdot 3}(x)=x+x+x$.
        \item
        Generalizing the above pattern, $F_{\omega\cdot n}(x)=x\cdot n$.
    \end{enumerate}
\end{example}
...
\label{epsilon0example}
    ...
    \begin{align*}
        F_{\omega^\omega}(x) &= x^x,\\
        F_{\omega^{\omega^\omega}+5}(x) &= x^{x^x}+5,\\
        F_{\omega^{\omega^{\omega^{\omega^\omega}}}}(x) &= x^{x^{x^{x^{x}}}},
    \end{align*}
    and so on. Likewise, $(\omega,\omega^\omega,\omega^{\omega^\omega},\ldots)$ is itself a
    natural fundamental sequence for $\epsilon_0$. What can $F_{\epsilon_0}(x)$ be?
    Thinking of $\epsilon_0$ as $\omega^{\omega^{\iddots}}$, one might initially think
    $F_{\epsilon_0}(x)=x^{x^{\iddots}}$, but such an infinite tower of natural numbers
    does not make sense for $x>1$. Instead, the answer defies familiar mathematical notation,
    but can be indicated as follows:
    \begin{align*}
        F_{\epsilon_0}(0) &= 0\\
        F_{\epsilon_0}(1) &= 1^1\\
        F_{\epsilon_0}(2) &= 2^{2^2}\\
        F_{\epsilon_0}(3) &= 3^{3^{3^3}},
    \end{align*}
    and so on.
\end{example}

Example \ref{epsilon0example} illustrates how the slow-growing hierarchy reduces
the open-ended problem, ``Invent a quickly-growing function,'' into a slightly
