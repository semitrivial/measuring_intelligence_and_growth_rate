...
\begin{example}
    ...
    \begin{enumerate}
        ...
        and in general each $f(n)=e_{n+1}$ for some such $e_1,e_2,\ldots$ as just described.
        Then:
        \begin{enumerate}
            \item
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        $B(k)=\cup_{n\in\mathbb N}B(f(n))$.
    \end{enumerate}
    For $k\in\mathcal O$, we say that $k$ is \emph{non-redundant} if
    for every $\alpha<|k|$, there is exactly one element $k_0\in B(k)$
    such that $|k_0|=\alpha$.
\end{definition}

\begin{definition}
    By a \emph{non-redundant ordinal}, we mean an ordinal of the form $|k|$
    for some non-redundant $k\in\mathcal O$.
\end{definition}

\begin{openquestion}
    Are the non-redundant ordinals exactly the computable ordinals?
\end{openquestion}
...
\begin{definition}
    ...
    \begin{enumerate}
        \item
        By $INC$, we mean the set of all $n\in\mathbb N$ such that the
        $n$th Turing machine computes a total computable function $f:\mathbb N\to\mathbb N$
        such that $f$ is increasing.
        \item
        Suppose $\alpha$ is a non-redundant ordinal.
        By a \emph{majorization hierarchy up to $\alpha$}, we mean a function
        $F:\alpha\to INC$ such that there is some non-redundant $k$ and some total
        computable function $g:\mathbb N\to\mathbb N$ such that:
        \begin{enumerate}
            \item
            $|k|=\alpha$.
            \item
            For all $k_0\in B(k)$, $f(|k_0|)=g(k_0)$.
            \item
            For all $\gamma<\beta<\alpha$, $F(\beta)\succ F(\gamma)$
            (Definition \ref{functionsuccdefn}).
        \end{enumerate}
        \item
        By an \emph{encoding} of a majorization hierarchy $F$ up to $\alpha$,
        we mean a pair $(k,M)$ where $k\in\mathcal O$ is as above
        and $M$ is a Turing machine that computes some $g:\mathbb N\to\mathbb N$
        as above.
    \end{enumerate}
\end{definition}

\begin{definition}
\label{majorizationhierarchiesofanagidefn}
    For each AGI $X$, let $(k^X_1,M^X_1),(k^X_2,M^X_2),\ldots$ be the
    sequence of majorization-hierarchy-encodings which would result
    if $X$ were commanded:
    \begin{quote}
        ``Until further notice, list encodings of majorization hierarchies
        $F_1,F_2,\ldots$ up to ordinals $\alpha_1,\alpha_2,\ldots$ in such a way that
        each $\alpha_{i+1}>\alpha_i$; and in such a way that each $F_{i+1}$ extends $F_i$;
        and in such a way that for every $k$ such that you know $k$ is a non-redundant
        member of $\mathcal O$, there is some $i$ with $\alpha_i>|k|$;
        and in such a way that for every Turing machine $C$ such that you know $C$
        computes a total function $f:\mathbb N\to\mathbb N$, there is some
        $i$ and some $\alpha<\alpha_i$ such that $F_i(\alpha)\succ f$.''
    \end{quote}
    Let $F^X_1:\alpha^X_1\to INC, F^X_2:\alpha^X_2\to INC, \ldots$
    be the majorization hierarchies encoded by $(k^X_1,M^X_1),(k^X_2,M^X_2),\ldots$.
\end{definition}

A priori, the reader would be justified in questioning whether the task above is
necessarily \emph{possible} for an AGI to carry out. The following lemma will show that it is.

\begin{lemma}
    Let $X$ be an AGI. It is possible for $X$ to carry out the command in
    Definition \ref{majorizationhierarchiesofanagidefn}.
\end{lemma}
