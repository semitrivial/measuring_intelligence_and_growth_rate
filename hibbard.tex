...
\begin{proof}
    ...
    satisfying conditions 1--4 of the command in
    Definition \ref{majorizationhierarchiesofanagidefn}.
    ...
    still satisfy conditions 1--4 of the command in
    Definition \ref{majorizationhierarchiesofanagidefn}.
    ...
    it follows (by choice of $\ell_1,\ell_2,\ldots$) that condition 5 of the command from
    Definition \ref{majorizationhierarchiesofanagidefn} is satisfied.
    ...
    that condition 6 of the command from
    Definition \ref{majorizationhierarchiesofanagidefn} is satisfied.

    So $(k_1,F_1),(k_2,F_2),\ldots$, as defined above, satisfy all the requirements
    of the command from Definition \ref{majorizationhierarchiesofanagidefn}.
...
\begin{definition}
    Suppose $X$ is an AGI. We define the \emph{majorization hierarchy of $X$} as follows.
    Let $(k^X_1,F^X_1),(k^X_2,F^X_2),\ldots$ be as in
    Definition \ref{majorizationhierarchiesofanagidefn}.
    Let $k=3\cdot 5^e$ where $e$ is such that the $e$th Turing machine defines the
    function $f:\mathbb N\to\mathbb N$ given by $f(n)=k^X_{n+1}$ (such an $e$ exists
    by the \emph{mechanicalness} part of Assumption \ref{idealizingassumption}).
    Define $F:B(k)\to INC$ so that for each $k'\in B(k)$ and each $g\in INC$,
    $F(k')=g$ if and only if $F^X_i(k')=g$ for some $i$ (this makes $F$ a
    function from $B(k)$ to $INC$ by conditions 3--4 of the command in Definition
    \ref{majorizationhierarchiesofanagidefn}, and $F$ is computable because
    the $F^X_1,F^X_2,\ldots$ can be dovetailed until one of them halts on input
    $k'$). The majorization hierarchy of $X$ is defined to be the majorization
    hierarchy $(k,F)$.
\end{definition}

\subsection{The transfinite Hibbard measure defined by an AGI}
