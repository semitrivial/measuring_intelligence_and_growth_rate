...
\begin{proposition}
    For each $v\in\mathrm{range}(H)$, let
    \[
        E^H_v = \{e\,:\,\mbox{$e$ is an evader and $H(t_e)< v$}\}
    \]
    be the set of evaders whose runtimes have growth rate $< v$ according to
    $H$.
    For any predictor $p$, the original Hibbard measure of $p$
    is equal to the largest $v\in\mathrm{range}(H)$ such that
    $p$ learns $e$ whenever $e\in E^H_v$.
\end{proposition}
...
\begin{proof}
    ...
    Orig(p) = max m s.t. p learns e whenever t_e in E_{f_m}
            = max m s.t. p learns e whenever f_m \succ t_e
            = max m s.t. p learns e whenever H(t_e) < m

    Orig(p) = infinity because p learns e whenever t_e in E_{f_m} for any m
            = infinity because p learns e whenever f_m \succ 
\end{proof}
