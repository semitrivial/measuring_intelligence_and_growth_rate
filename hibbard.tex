...
\begin{theorem}
    ...
    Suppose $X$ and $Y$ are AGIs. Let $e$ be the brute-force evader of $Y$.
    If $X$ knows that $e$ computes a total function from $B^*$ to $B$, then
    $|Y|_X$ is finite.
\end{theorem}

\begin{proof}
    Since $X$ knows that $e$ computes a total function from $B^*$ to $B$,
    it follows that there is a Turing machine $C$ such that:
    \begin{enumerate}
        \item $C$ computes $t_e:\mathbb N\to\mathbb N$ (from Definition \ref{tsubedefinition}).
        \item $X$ knows that $C$ defines a total function from $\mathbb N$ to $\mathbb N$.
    \end{enumerate}
    By (2), $C=C^X_k$ for some $k$, and thus $g^X_k=t_e$.
