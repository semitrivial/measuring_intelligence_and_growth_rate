...
\begin{lemma}
\label{fairsequencelemma}
    For every AGI $X$, $h^X_1,h^X_2,\ldots$ are isomorphic to an ordinal,
    by which we mean the following. Let $S=\{h^X_1,h^X_2,\ldots\}$.
    There is some computable ordinal $\alpha$ and some function $f:S\to \alpha$
    such that:
    \begin{enumerate}
        \item
        For every $\beta<\alpha$, there is exactly one $i$ such that $f(h^X_i)=\beta$.
        \item
        For all $i,j>0$, $h^X_i\succ h^X_j$ if and only if $f(h^X_i)>f(h^X_j)$.
    \end{enumerate}
\end{lemma}

\begin{proof}
    Immediate by the \emph{linear ordering} and \emph{well-foundedness} conditions
    in the command from Definition \ref{fairsequencedefn}.
\end{proof}
...
\begin{definition}
    ...
    intelligence measure $\|Y\|_X$ defined as follows. Let $f$ be as in
    Lemma \ref{fairsequencelemma}. We define $\|Y\|_X$ to be
    \[
        \|Y\|_X=\sup\{f(i)\,:\,\mbox{$Y$ learns every evader in $E_{h^X_i}$}\}.
    \]
\end{definition}
