...
\label{Xspredictordefn}
    If $X$ is an AGI, \emph{$X$'s predictor} is the predictor which
...
(Definition \ref{bruteforcepredictordefn}).
...
\begin{proof}
    ...
    at most finitely many $M_i$'s can be ruled out from being the evader,
    which implies that after finitely many failures, $p$ always succeeds at
    predicting $e$, that is, $p$ learns to predict $e$, as desired.
\end{proof}
