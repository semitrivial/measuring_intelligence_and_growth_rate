...
cross-sections are probably examples of so-called \emph{AI-Complete} problems
\cite{yampolskiy2012ai}.
...
then $X$ is more intelligent than $Y$ (more precisely: $X$ is more intelligent
than $Y$ in the sense that $|X|_X=\infty>|Y|_X$).

We should therefore expect that this property should hold for bigger groups of AGIs:
we should expect the following:
\begin{quote}
    (Desideratum) For any AGIs $X,Y,Z$, if $X$ knows the both $Y$'s predictor
    and $Z$'s predictor compute total functions from $B^*$ to $B$, and if
    $Y$ knows that $Z$'s predictor computes a total function from $B^*$ to $B$,
    then we should expect that $|X|_X=\infty>|Y|_X>|Z|_X$.
\end{quote}
Unfortunately, this almost surely cannot be the case in general, and the reason has
almost nothing to do with intelligence or with adversarial sequence prediction,
and almost entirely to do with the structure of the natural numbers: relative Hibbard
measures assign natural-number-valued intelligence measures to AGIs, and those
values therefore suffer the same limitations as the natural numbers.

Let $Y_0$ be some initial AGI.
\begin{itemize}
    \item
    By mechanicalness, $Y_0$'s predictor $p_0$ computes a total function from $B^*$ to $B$.
    \item
    If $Y_0$ is capable of being taught new facts (and thereby becoming a new AGI),
    then we should be able to define $Y_1$ to be the AGI which would result from making
    a clone of $Y_0$ and teaching the clone that $p_0$ computes a total function from $B^*$
    to $B$.
    \item
    By mechanicalness, $Y_1$'s predictor $p_1$ computes a total function from $B^*$ to $B$.
    \item
    If $Y_1$ is capable of being taught new facts (and thereby becoming a new AGI),
    then we should be able to define $Y_2$ to be the AGI which would result from making
    a clone of $Y_1$ and teaching the clone that $p_1$ computes a total function from $B^*$
    to $B$.
    \item
    Repeating the above process, we should be able to define AGIs $Y_0,Y_1,Y_2,\ldots$
    such that if each $Y_i$ has predictor $p_i$, then each $Y_{i+1}$ knows that $p_i$
    computes a total function from $B^*$ to $B$.
    \item
    Apparently nothing in the above process is non-mechanical, so by suitably formalizing
    the above process, we should be able to define a Turing machine $M$ that enumerates
    $p_0,p_1,\ldots$. It is then a true fact that $M$ enumerates an infinite
    list of Turing machines
    each one of which defines a total function from $B^*$ to $B$.
    \item
    If $Y_0$ is capable of being taught new facts (and thereby becoming a new AGI),
    then we should be able to define $X$ to be the AGI which would result from making
    a clone of $Y_0$ and teaching the clone that $M$ enumerates an infinite list of
    Turing machines each one of which defines a total function from $B^*$ to $B$.
    \item
    In this way, for every $i$, $Y$ knows that $p_i$ defines a total function from $B^*$
    to $B$. Let $p$ be $Y$'s predictor.
    \item
    Finally, let $X$ be the AGI which would result from making a clone of $Y$ and teaching
    the clone that $p$ predictor computes a total function from $B^*$ to $B$.
\end{itemize}
So in summary: each $Y_i$ has predictor $p_i$, each $Y_{i+1}$ knows that
$p_i$ computes a total function from $B^*$ to $B$, $Y$ has predictor $p$,
$Y$ knows that all of the $p_i$ compute total functions from $B^*$ to $B$,
and $X$ knows that $p$ computes a total function from $B^*$ to $B$ and that all the
$p_i$ do too.
If the above \emph{Desideratum} held in general, it would force $|Y|_X<\infty$
and at the same time $|Y_0|_X<|Y_1|_X<|Y_2|_X<\cdots$ and at the same time
each $|Y_i|_X<|Y|_X$. This cannot hold, because $|\bullet|_X$ is natural-number-valued:
if $|Y|_X$ were, say, $999$, then $|Y_0|_X,\ldots,|Y_{1000}|_X$ would be
a length-$1001$ increasing sequence of naturals $<999$, which is absurd.

The above example shows if the \emph{Desirata} held,
then AGI intelligence would be \emph{non-Archimedean},
in a sense which we make formal in \cite{alexander2020archimedean}.
In that paper, we argue that it is hopeless to try to measure non-Archimedean
structures using natural numbers---in fact, that it is even hopeless to measure
non-Archimedean structures using \emph{real} numbers. If AGI intelligence
is non-Archimedean, then in order to accurately measure it, we need to use
a non-Archimedean number system. In Section \ref{transfinitehibbardsection}
