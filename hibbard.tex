...
\begin{definition}
    By a \emph{majorization hierarchy}, we mean a pair $(k,M)$
    where:
    \begin{enumerate}
        \item
        $k\in\mathcal O$.
        ...
        $k$ is non-redundant.
        \item
        $M$ is a Turing machine which computes a
        total computable function $f:\mathbb N\to\mathbb N$ such that:
        \begin{enumerate}
            \item
            For every $m\in B(k)$, the $f(m)$th Turing machine computes a total
            function $g_m:\mathbb N\to\mathbb N$ such that $g_m$ is increasing.
            \item
            For every $m,n\in B(k)$, if $|m|>|n|$, then
            $g_m\succ g_n$ (Definition \ref{functionsuccdefn}).
        \end{enumerate}
    \end{enumerate}
    If $(k_1,M_1)$ and $(k_2,M_2)$ are two majorization hierarchies,
    we say that $(k_2,M_2)$ \emph{extends} $(k_1,M_1)$ if
    $k_2=3\cdot 5^e$ for some $e$ such that the $e$th Turing machine
    computes a function $f:\mathbb N\to\mathbb N$ such that $f(0)=k_1$.
\end{definition}
