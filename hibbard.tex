...
\begin{proof}
    ...
    Given $n\geq 1$, let $p$ be the predictor who proceeds as follows when given
    ...
    \begin{itemize}
        \item $T_i$ halts and outputs $x_1$ on input $\langle\rangle$
            within $\leq f_m(n)$ steps.
        \item $T_i$ halts and outputs $x_2$ on input $(y_1)$
            within $\leq f_m(n)$ steps.
        \item $T_i$ halts and outputs $x_3$ on input $(y_1,y_2)$
            within $\leq f_m(n)$ steps.
        \item $\ldots$
        \item $T_i$ halts and outputs $x_n$ on input $(y_1,\ldots,y_{n-1})$
            within $\leq f_m(n)$ steps.
        \item $T_i$ halts with some output $Y_{i,n}$ on input $(y_1,\ldots,y_n)$
            within $\leq f_m(n)$ steps.
    \end{itemize}
    By simulating $T_1,\ldots,T_n$ for $f_m(n)$ steps on each input,
    $p$ determines if any $T_i$ is an $n$th-order $e$-lookalike.
    If so, $p$ outputs $p(x_1,\ldots,x_n)=Y_{i,n}$ for the minimal such $i$.
    If not, $p$ outputs $0$.

    To show that $p$ has original Hibbard measure $\geq m$,
    let $e\in E_{f_m}$, we will show $p$ learns $e$.
    Being in $E_{f_m}$, $e$ is an evader and $f_m\succ t_e$.
    ...
    $n$th-order $e$-lookalike and so plays $Y_{k',n}$ but $e(y_1,\ldots,y_n)\not=Y_{k',n}$
    ...
    \begin{enumerate}
        \item
        There is some $k'<k$ such that, for all sufficiently large $n$,
        on input $(x_1,\ldots,x_n)$, $p$ identifies $T_{k'}$ as the first $n$th-order
        $e$-lookalike, and does not get tricked by $T_{k'}$. Or,
        \item
        For all sufficiently large $n$, on input $(x_1,\ldots,x_n)$,
        $p$ identifies $T_k$ as the first $n$th-order $e$-lookalike.
    \end{enumerate}
    Either way, for all sufficiently large $n$, $p$ correctly guesses $e$'s next digit.
\end{proof}
