...
\begin{quote}
    ...
    \begin{enumerate}
        ...
        \item Rewrite 'caution' in such a way as to emphasize Reinhardt-style ignorance.
        ...
        \item Redefine simple Hibbard measures so that the agent is commanded to
...
\label{bruteforceassumption}
    For any AGI $X$, $X$'s predictor is at least as good as
    $X$'s brute-force predictor, by which we mean that for any evader $e$,
...
In Subsection \ref{trivialitysubsection}, we argued that every AGI probably has
...
the simple Hibbard measure given by $X$ does not assign intelligence $\infty$
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        $g(n)=t_e(n)+1$, where $t_e$ is as in Definition \ref{tsubedefinition}.
        \item
        $X$ knows that $C$ computes a total function from $\mathbb N$ to $\mathbb N$.
    \end{enumerate}
    By (2), $C=C^X_m$ for some $m>0$, and thus $g^X_m=g$.
...
\label{technicallemmaaboutsimplehibbardmeasures}
    Suppose $X$ and $Y$ are AGIs. Suppose that for all $i=1,\ldots,k$,
    $g^X_i$ is identically zero: $g^X_i(n)=0$.
    Then $|Y|_X\geq k$.
\end{lemma}

\begin{proof}
    If $f_k:\mathbb N\to\mathbb N$
    is defined (as in Definition \ref{generalintelligencemeasuredefn})
    by $f_k(n)=\max_{0<i\leq k}\max_{j\leq n}g^X_i(j)$, then, by
    assumption, $f_k(n)=\max_{0<i\leq k}\max_{j\leq n}0=0$.
    Thus $E_{f_k}$ is empty, and so, vacuously, $Y$'s predictor learns
    to predict every evader in $E_{f_k}$. This shows $|Y|_X\geq m$.
\end{proof}
...
as in Definition \ref{functionlistdefinition}. If we want $|Y|_{X'}$ to be exactly
equal to, say, $k$, we need only contrive that $C^{X'}_1,\ldots,C^{X'}_k$
compute the zero function and $C^{X'}_{k+1}$ computes $Y$'s predictor.

The above is a crucial flaw in the simple Hibbard measures, because it shows
...
we will define a more sophisticated type of Hibbard measure
...
\begin{itemize}
    \item A \emph{relation} is a set $R$ of pairs $(x,y)$. Whenever $(x,y)\in R$,
    we write $xRy$.
