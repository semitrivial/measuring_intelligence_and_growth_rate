...
\begin{definition}
    Let $X$ be an AGI. The \emph{transfinite Hibbard measure} given by $X$ is
    the function which assigns to each AGI $Y$ a computable ordinal $\|Y\|_X$
    defined as follows. Let $\alpha_\infty$ and $\mathcal F_\infty$ be as in
    Definition \ref{slowgrowinghierarchyofanagidefn}, and let
    $\{G_\beta\}_{\beta<\alpha_\infty}$ be the slow-growing hierarchy
