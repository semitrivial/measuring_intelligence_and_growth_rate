...
\begin{proof}
    ...
    as the first $n$th-order $e$-lookalike and so plays $X_{k',n}$ but
    $x_{n+1}\not=X_{k',n}$
    ...
    $p_f$ plays $X_{k',n}=x_{n+1}$, lest $p_f$ be tricked by $T_{k'}$), or else
    $p_f$ identifies $T_k$ as the first $n$th-order $e$-lookalike, in which case
    $p_f$ plays $X_{k,n}=x_{n+1}$ since $e=T_k$. Either way, after possibly being
    tricked finitely many times, $p_f$ always guesses correctly whenever
    ...
    whenever $f_m(n)>t_e(n)$, $x_n=y_n$. It follows that $p_{f_m}$ learns $e$,
    as desired.
\end{proof}
