...
The following theorem shows that the simple Hibbard measure given by an AGI $X$
depends non-trivially on the order in which $X$ would enumerate Turing machines
when commanded as in Definition \ref{functionlistdefinition}.

\begin{theorem}
    (Order-dependence of simple Hibbard measures)
    Suppose $X,Y,Z$ are AGIs. Let $p_Z$ and $p_Y$ be $Z$'s predictor and
    $Y$'s predictor, respectively. Assume the following:
    \begin{enumerate}
        \item $X$ knows that $p_Y$ computes a total function from $B^*$ to $B$.
        \item $Y$ knows that $p_Z$ computes a total function from $B^*$ to $B$.
    \end{enumerate}
    Then $|Y|_X$ and $|Z|_X$ may be equal or unequal, depending on the order
    in which $X$ would enumerate Turing machines when commanded as in
    Definition \ref{functionlistdefinition}.
\end{theorem}
